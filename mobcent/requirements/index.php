...
$requirements = array(
    ...
    //     t('yii', '会影响客户端的消息模块, 如未通过,<a href="http://addon.discuz.com/?@appbyme_app.plugin.doc/FAQ">请点击查看说明</a>'),
    // ),
    // array(
    ...
    array(
        ...
        t('yii', '图像处理库 扩展模块（有FreeType支持的 GD 库 或 <br /> 有PNG支持 ImageMagick 库）'),
        false,
    ...
    array(
        ...
        '',
        t('yii', '请保证 discuz 根目录下的 data 目录可写')
    ),
    array(
        t('yii', 'PHP 设置 allow_url_fopen'),
        true,
        checkAllowUrlFopen(),
        '',
    ...
    array(
        t('yii', 'PHP 设置 asp_tags'),
        true,
        checkAspTags(),
        '',
        t('yii', '请保证 php.ini 中的 asp_tags 为 Off 或者为 0')
    ),
);
...
function checkAllowUrlFopen() {
    ...
    return $allowUrlFopen == 1;
}

function checkAspTags() {
...
function checkCaptchaSupport() {
    ...
    return t('yii', 'GD 或者 ImageMagick 没有被安装, 会影响缩略图功能');
}
...
function checkDirectoryWritable() {
    return is_writable(dirname(__FILE__) . '/../../data');
}
...
if (isset($_GET['ajax'])) {
    echo json_encode(array('rs' => $result));
    exit;
}
