...
class TopicListAction extends MobcentAction {
    ...
    public function run($boardId = 0, $page = 1, $pageSize = 10, $sortby = 'all', $filterType = '', $filterId = 0, $isImageList = 0, $topOrder = 0, $circle = 0) {
        switch ($boardId) {
            case -1:
                $sortby = 'new';
                $boardId = 0;
        ...
        global $_G;
        $key = CacheUtils::getTopicListKey(array($fid, $_G['groupid'], $page, $pageSize, $sort, $filterType, $filterId, $isImageList, $topOrder, $circle, 'new'));
        $this->runWithCache($key, array('fid' => $fid, 'page' => $page, 'pageSize' => $pageSize, 'sort' => $sort, 'filterType' => $filterType, 'filterId' => $filterId, 'isImageList' => $isImageList, 'topOrder' => $topOrder, 'circle' => $circle));
    }
    ...
    protected function runWithCache($key, $params = array()) {
        ...
        if ($params['sort'] == 'photo') {
            ...
            if (!empty($fid)) {
                ...
                unset($res['forumInfo']);
                unset($res['topTopicList']);
            }
            $out = '';
        }
        WebUtils::outputWebApi($res, $out);
    }
    ...
    private function _getTopicInfos($fid, $page, $pageSize, $sort, $filterType = '', $filterId = '', $isImageList = '', $topOrder = '') {
        ...
        if ($page == 1 && $topOrder != 0) {
            ...
            if ($status != '1') {
                ...
                $topTopicListTmp = $this->_getTopicList($fid, 1, 4, 'top', $filterType, $filterId);
    ...
    private function _getListField($topicList) {
        ...
        foreach ($topicList as $topic) {
            ...
            if ($topic['author'] == NULL) {  //Fix 匿名显示头像和可点击 ByNaiXiaoXin Data 20150910
                global $_G;
