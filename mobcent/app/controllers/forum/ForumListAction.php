...
class ForumListAction extends MobcentAction {
    ...
    private function _getForumList($fid, $type) {
        ...
        if ($fid > 0) {
            ...
            $tempForum['description'] = (string) WebUtils::emptyHtml($tempForumInfo['description']);
            $tempForum['icon'] = (string) $forumImage;
            $tempForum['td_posts_num'] = $tempForumInfo['todayposts'];
            $tempForum['topic_total_num'] = $tempForumInfo['threads'];
            $tempForum['posts_total_num'] = $tempForumInfo['posts'];
            global $_G;
            $tempForum['is_focus'] = (int) ForumUtils::getFavStatus($_G['uid'], $fid);
    ...
    private function _getForumInfo($forum, $params = array()) {
        ...
        $forumInfo['favNum'] = (int) $forum['favtimes'];
