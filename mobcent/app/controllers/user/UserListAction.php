...
class UserListAction extends CAction {
    ...
    // 设置用户关注
    private function _getRecommendUserList($uid, $page, $pageSize, $sortType, $longitude, $latitude, $radius, $gender) {
            $userListInfo = array();
            $userListInfo['users'] = $this->_getAllUidList($uid, $page, $pageSize, $sortType, $longitude, $latitude, $radius, $gender);
            $userListInfo['count'] = DzCommonUserListGender::_getRecommendUsersCountGender($uid, $gender);
            return $userListInfo;
    }
    //用户关注排序，增加男女筛选
