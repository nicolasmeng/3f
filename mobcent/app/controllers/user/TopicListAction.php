...
class TopicListAction extends CAction {
    ...
    public function run() {
        ...
        $res = $this->_getTopicList($res, $type, $idType, $uid, $page, $pageSize);

        echo WebUtils::outputWebApi($res, '', false);
    }

    private function _getTopicList($res, $type, $idType, $uid, $page, $pageSize) {
        $res['list'] = $this->_getUserTopicList($type, $idType, $uid, $page, $pageSize);
        $count = $this->_getUserTopicListCount($type, $idType, $uid);
    ...
    private function _transTopicList($topicList, $type) {
        ...
        foreach ($topicList as $topic) {
            ...
            if ($_GET['isImageList'] == '1') {
                $topicSummary = ForumUtils::getTopicSummary((int)$topic, 'forum', true, array('imageList' => 1, 'imageListLen' => 9, 'imageListThumb' => 1));
                $topicInfo['imageList'] = (array)$topicSummary['imageList'];
            } else {
                $topicSummary = ForumUtils::getTopicSummary((int)$topic);
                $image = ImageUtils::getThumbImageEx($topicSummary['image'],15,true);
                $topicInfo['pic_path'] = $image['image'];
            }
            $topicInfo['board_id'] = (int)$tmpTopicInfo['fid'];
            $topicInfo['board_name'] = ForumUtils::getForumName($tmpTopicInfo['fid']);
            ...
            $topicInfo['userAvatar'] = UserUtils::getUserAvatar($tmpTopicInfo['authorid']);
            $topicInfo['special'] = (int)$tmpTopicInfo['special'];
            $list[] = $topicInfo;
        }
        return $list;
    }
    ...
    // 获取发表、回复和关注帖子的总数
    private function _getUserTopicListCount($type, $idType, $uid) {
        switch ($type) {
            case 'topic':
