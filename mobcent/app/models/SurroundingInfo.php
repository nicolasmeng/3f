...
class SurroundingInfo extends DiscuzAR {
    ...
    public static function getUserLat($uid) {
        $res = DbUtils::getDzDbUtils(true)->queryRow(
            "SELECT
                longitude,latitude
             FROM
                %t
            WHERE
                object_id = %d
            ",
            array('home_surrounding_user',$uid)
        );
        return $res;
    }
    public static function getSeachuser($longitude, $latitude,$uid) {
        $select = 'location, ' . self::_getSqlDistance($longitude, $latitude) . ' AS distance';
        $res = DbUtils::getDzDbUtils(true)->queryRow(
            "SELECT
                $select
             FROM
