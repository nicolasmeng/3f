...
class UserUtils {
     ...
     * @return array .
     */
    public static function register($username, $password, $email, $type = 'otherReg', $FastRegister = 0) {
        global $_G;
        require_once libfile('function/member');
        ...
        if ($limit !== true) {
            ...
            if (!is_array($action)) {
                if (!$fid) {
					...
					if($_G['forum']['fup'] == 36){ $res['msg'] = 'mobcent_credits_policy_lowerlimit_fid_3f7s';
                    $res['values'] = $values;}
				    else{ $res['msg'] = 'mobcent_credits_policy_lowerlimit_fid';
                    $res['values'] = $values;}
