...
class AppUtils {
    ...
    public static function filterComponent($component) {
        ...
        //过滤新增自定义
        if($tempComponent['style'] == AppbymeUIDiyModel::COMPONENT_TYPE_TRANSPARENT){
            foreach($tempComponent['componentList'] as $key => $val){
                if($val['style'] == AppbymeUIDiyModel::COMPONENT_TYPE_SUBNAVFLAT){
                    foreach($val['componentList'] as $k => $v){
                        if($v['type'] == 'empty'){
                            unset($tempComponent['componentList'][$key]['componentList'][$k]);
                        }
                    }
                }
            }
        ...
        if ($tempComponent['style'] == AppbymeUIDiyModel::COMPONENT_STYLE_LAYOUT_NEWS_AUTO &&
            ...
            if ($newslist = WebUtils::tarr(WebUtils::jsonDecode($newslist))) {
                foreach ($newslist['list'] as $key => $value) {
                    ...
                    $tempParam['extParams'] = array_merge($tempParam['extParams'], $value);
