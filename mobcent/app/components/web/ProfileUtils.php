...
class ProfileUtils {
     ...
     * 校验注册时填写的表单信息
     */
    public function checkReg($userInfo) {
        ProfileUtils::init();
        global $_G;
        $rs = WebUtils::initWebApiResult();
        //版本号过低 直接返回成功
        if ($_GET['sdkVersion'] < '3.0.0')
            return $rs;
        $censor = discuz_censor::instance();
        $set = ProfileUtils::_getRegSetting();
        $return = array();
        foreach ($set as $k => $s) {
            $field = $_G['cache']['profilesetting'][$k];
            //必填
            if (empty($userInfo[$k]) && !$s['must']) {
                $msg = WebUtils::u($field['title']) . '必须填写！';
                $rs['errCode'] = 1;
                $rs['errMsg'] = $msg;
                return $rs;
            }
            ...
            if ($check['errCode'] != 0) {
                ...
                $return = array_merge($return . $rs['return']);
            }
        }
        $rs['return'] = $return;
        return $rs;
    ...
    public function _doSpace($field, $info) {
        ...
        if ($field['fieldid'] == 'birthcity') {
            ...
            space_merge($space, 'field_home');
            space_merge($space, 'profile');
            if (isset($info['birthmonth']) && ($space['birthmonth'] != $info['birthmonth'] || $space['birthday'] != $info['birthday'])) {//计算星座
                $return['constellation'] = get_constellation($info['birthmonth'], $info['birthday']);
            }
            if (isset($info['birthyear']) && $space['birthyear'] != $info['birthyear']) {// 计算生肖
