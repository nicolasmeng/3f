...
<?php global $_G; ?>
            ...
            <div class="col-sm-xs-8 col-sm-8 col-md-8" id="operation">
                ...
                <div class="uidiy-config-admin">
                    <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <a href="<?php echo WebUtils::createUrl_oldVersion('admin/uidiy/exportconfig',array('id'=>$id)) ?>" class="btn btn-default btn-xs">导出配置</a> (仅导出当前已保存的配置)
                        </div>
                    </div>
                </div>
                ...
                <div class="text-left">
                    <p>
                        ...
                        <input type="hidden" id="uidiyid"  value="<?php echo$id ?>"> 
                ...
                <div class="panel panel-primary">
                    <div class="panel-heading">
                        <h3 class="panel-title">选择导航样式     <?php if(!empty($id)){ ?><B><font color=red>您正在编辑<?php echo WebUtils::u($name); ?> ID：<?php echo $id; ?> </font></B>  <?php } ?> </h3>
                    </div>
                    <div class="panel-body">
                        <div class="checkbox">
                            <label class="align-text"><input type="radio" checked> 底部导航</label>
                            <select id="allVersionNum" name="allVersionNum">
                            <?php
                            $versions = AppbymeUIDiyModel::getVersionConfig();
                            foreach ($versions as $k => $v){
                                $select = $k == $version ? 'selected' : ''; 
    ...
    ?>
    var SUBNAV_MAX_COMPONENT_LEN = 8;
    var NOW_VERSION = '<?php echo $version;?>';
    </script>
    ...
    <script src="<?php echo $this->rootUrl; ?>/js/admin/uidiy.js?v=20160509"></script>
    <!-- 底部导航模板 -->
        ...
        <div class="panel-body">
            ...
            <div class="<%= !isModuleTypeSelect ? 'hidden' : '' %> form-group" style="position: relative;">
                ...
                <select id="moduleType" name="moduleType" class="form-control">
                    ...
                    <% if (typeof NOW_VERSION != 'undefined' && NOW_VERSION >= 2) {%>
                    <option value="<%= MODULE_TYPE_NEWCUSTOM %>" <%= type == MODULE_TYPE_NEWCUSTOM ? 'selected' : '' %>>新版自定义</option>
                    <%}%>
                </select>
                </div>
            ...
            <div class="form-group module-style-select-div">
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="moduleStyle">
                        ...
                        <option value="<%= COMPONENT_STYLE_SUBNAV_TOPBAR %>" <%= style == COMPONENT_STYLE_SUBNAV_TOPBAR ? 'selected' : '' %>>顶部导航样式</option>
                    </select> 
                </div>
            </div>
    ...
    <% } else if (type == MODULE_TYPE_NEWCUSTOM) { %>
    <!-- 新增加风格 -->
    <div class="add-style">
        <div class="custom-style-item-container">
        </div>
        ...
        <div class="form-group fastpost-item-select-div hidden">
            <div class="col-sm-offset-2 col-sm-7">
                ...
                <select class="input-sm" name="fastpostItemSelect">
                    ...
                    <option value="<%= COMPONENT_TYPE_WEBAPP %>">外部wap页</option>
                </select>                        
    ...
    <div class="quick-edit" id="component-view-<%= id %>">
        <div class="form-group <%= this.uiconfig.isShow_px ? '' : 'hidden' %>">
            <label for="" class="col-sm-2 control-label">排序: </label>
            <div class="col-sm-10">
                <input type="text" class="form-control input-sm" name="componentPx[]" value="<%= px  %>">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="componentType[]" class="selectComponentType form-control" <%= this.uiconfig.isShow_typeSelect ? '' : 'disabled' %>>
                <option value="<%= COMPONENT_TYPE_EMPTY %>" <%= type == COMPONENT_TYPE_EMPTY ? 'selected' : '' %> class="<%= this.uiconfig.isShow_typeEmpty ? '' : 'hidden' %>">无</option>
                ...
                <% if (typeof NOW_VERSION != 'undefined' && NOW_VERSION >= 2) {%>
                <option value="<%= COMPONENT_TYPE_TOPICLIST_COMPLEX %>" <%= type == COMPONENT_TYPE_TOPICLIST_COMPLEX ? 'selected' : '' %> class="<%= this.uiconfig.isShow_typeTopiclistComplex ? '' : 'hidden' %>">复杂帖子列表</option>
                <%}%>
                ...
                <% if (typeof NOW_VERSION != 'undefined' && NOW_VERSION >= 2) {%>
                <option value="<%= COMPONENT_TYPE_TALKPOSTLIST %>" <%= type == COMPONENT_TYPE_TALKPOSTLIST ? 'selected' : '' %> class="<%= this.uiconfig.isShow_typeTopiclisttalk ? '' : 'hidden' %>">话题帖子列表</option>
                <option value="<%= COMPONENT_TYPE_TALK %>" <%= type == COMPONENT_TYPE_TALK ? 'selected' : '' %> class="<%= this.uiconfig.isShow_typeTalklist ? '' : 'hidden' %>">话题列表</option>
                <%}%>
                ...
                <option value="<%= COMPONENT_TYPE_SCAN %>" <%= type == COMPONENT_TYPE_SCAN ? 'selected' : '' %> class="<%= this.uiconfig.isShow_typeScan ? '' : 'hidden' %>">二维码扫描</option>
                <% if (typeof NOW_VERSION != 'undefined' && NOW_VERSION >= 2) {%>
                <option value="<%= COMPONENT_TYPE_FASTTALK %>" <%= type == COMPONENT_TYPE_FASTTALK ? 'selected' : '' %> class="<%= this.uiconfig.isShow_typeFasttalk ? '' : 'hidden' %>">发表话题</option>
                <option value="<%= COMPONENT_TYPE_CONTACTS %>" <%= type == COMPONENT_TYPE_CONTACTS ? 'selected' : '' %> class="<%= this.uiconfig.isShow_typeContacts ? '' : 'hidden' %>">通讯录</option>
                <%}%>
                <option value="<%= COMPONENT_TYPE_LIVELIST %>" <%= type == COMPONENT_TYPE_LIVELIST ? 'selected' : '' %> class="<%= this.uiconfig.isShow_typeLivelist ? '' : 'hidden' %>">直播间</option>
                <option value="<%= COMPONENT_TYPE_CLASSIFICATIONLIST %>" <%= type == COMPONENT_TYPE_CLASSIFICATIONLIST ? 'selected' : '' %> class="<%= this.uiconfig.isShow_typeClassificationlist ? '' : 'hidden' %>">分类筛选</option>
                <option value="<%= COMPONENT_TYPE_CONFIG_SWITCH %>" <%= type == COMPONENT_TYPE_CONFIG_SWITCH ? 'selected' : '' %> class="<%= this.uiconfig.isShow_typeConfigSwitch ? '' : 'hidden' %>">配置切换</option>
            </select>
            </div>
        </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="forumTopiclistStyle[]">
                        ...
                        <option value="<%= COMPONENT_STYLE_CIRCLE %>" <%= extParams.subListStyle == COMPONENT_STYLE_CIRCLE ? 'selected' : '' %>>圈子样式</option>
                    </select> 
                </div>
            </div>
            <div class="form-group">
        ...
        <div id="component-view-<% print(COMPONENT_TYPE_NEWSLIST+'-'+id) %>" class="component-view-item <%= type == COMPONENT_TYPE_NEWSLIST ? '' : 'hidden' %>">
            <div class="form-group">
                <label for="" class="col-sm-2 control-label">高级列表模块: </label>
                <div class="col-sm-10">
        ...
        <div id="component-view-<% print(COMPONENT_TYPE_TOPICLIST_COMPLEX+'-'+id) %>" class="component-view-item <%= type == COMPONENT_TYPE_TOPICLIST_COMPLEX ? '' : 'hidden' %>">
            <div class="form-group">
                <label for="" class="col-sm-2 control-label">选择版块: </label>
                <div class="col-sm-10">
                    <select class="form-control topicComplexForumSelect" name="topicComplexForumId[]">
                        <option value="0" <%= extParams.forumId == 0 ? 'selected' : '' %> class="">无</option>
                        <?php foreach ($forumList as $fid => $title) { ?>
                        <option value="<?php echo $fid ?>" <%= extParams.forumId == <?php echo $fid; ?> ? 'selected' : '' %>><?php echo WebUtils::u($title) ?></option>
                    <?php } ?>
                    </select>
        ...
        <div id="component-view-<% print(COMPONENT_TYPE_TALK+'-'+id) %>" class="component-view-item <%= type == COMPONENT_TYPE_TALK ? '' : 'hidden' %>">
            <div class="form-group">
                <label class="col-sm-2 control-label">帖子列表页面样式: </label>
                <div class="col-sm-10">
                    <select class="form-control" name="forumTopiclistStyleTalk[]">
                        <option value="<%= COMPONENT_STYLE_FLAT %>" <%= extParams.subListStyle == COMPONENT_STYLE_FLAT ? 'selected' : '' %>>扁平样式</option>
                        <option value="<%= COMPONENT_STYLE_CARD %>" <%= extParams.subListStyle == COMPONENT_STYLE_CARD ? 'selected' : '' %>>卡片样式</option>
                        <option value="<%= COMPONENT_STYLE_TIEBA %>" <%= extParams.subListStyle == COMPONENT_STYLE_TIEBA ? 'selected' : '' %>>类贴吧样式</option>
                        <option value="<%= COMPONENT_STYLE_NETEASE_NEWS %>" <%= extParams.subListStyle == COMPONENT_STYLE_NETEASE_NEWS ? 'selected' : '' %>>类网易新闻样式</option>
        ...
        <div id="component-view-<% print(COMPONENT_TYPE_TALKPOSTLIST+'-'+id) %>" class="component-view-item <%= type == COMPONENT_TYPE_TALKPOSTLIST ? '' : 'hidden' %>">
            <div class="form-group">
                <label for="" class="col-sm-2 control-label">选择话题: </label>
                <div class="col-sm-10">
                    <select class="form-control topicComplexForumSelect" name="topicTalkId[]">
                        <option value="0" <%= extParams.talkId == 0 ? 'selected' : '' %> class="">无</option>
                        <?php foreach ($topic as $tval) { ?>
                        <option value="<?php echo $tval['ti_id'] ?>" <%= extParams.talkId == <?php echo $tval['ti_id']; ?> ? 'selected' : '' %>><?php echo WebUtils::u($tval['ti_title']) ?></option>
                    <?php } ?>
                    </select>
                </div>
            </div>
        ...
        <div id="component-view-<% print(COMPONENT_TYPE_SCAN+'-'+id) %>" class="component-view-item <%= type == COMPONENT_TYPE_SCAN ? '' : 'hidden' %>">
        </div>
        <div id="component-view-<% print(COMPONENT_TYPE_LIVELIST+'-'+id) %>" class="component-view-item <%= type == COMPONENT_TYPE_LIVELIST ? '' : 'hidden' %>">
        </div>
        <div id="component-view-<% print(COMPONENT_TYPE_CLASSIFICATIONLIST+'-'+id) %>" class="component-view-item <%= type == COMPONENT_TYPE_CLASSIFICATIONLIST ? '' : 'hidden' %>">
        </div>
        <div id="component-view-<% print(COMPONENT_TYPE_CONFIG_SWITCH+'-'+id) %>" class="component-view-item <%= type == COMPONENT_TYPE_CONFIG_SWITCH ? '' : 'hidden' %>">
        </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    ...
                    <% for (var i = 0; i < Appbyme.uiModules.length; i++) { %>
                        ...
                        <% if ((!isOnlyCustomModule && module.type != MODULE_TYPE_FASTPOST) || (isOnlyCustomModule && module.type == MODULE_TYPE_CUSTOM) || (isOnlyCustomModule && module.type == MODULE_TYPE_NEWS) || (isOnlyCustomModule && module.type == MODULE_TYPE_NEWCUSTOM)) { %>
        ...
        <div class="list-ext-config-div">
            <div class="form-group">
                <label class="col-sm-2 control-label">是否显示来自版块: </label>
                <div class="col-sm-10">
                    <select class="form-control" name="listBoardNameState[]">
                        <option value="<%= BOARD_NAME_GONE %>" <%= extParams.listBoardNameState == BOARD_NAME_GONE ? 'selected' : '' %>>否</option>
                        <option value="<%= BOARD_NAME_VISIBLE %>" <%= extParams.listBoardNameState == BOARD_NAME_VISIBLE ? 'selected' : '' %>>是</option>
                    </select>
                </div>
            </div>
            <div class="form-group">
        ...
        <!--tan_start-->
        <div class="list-ext-config-div-new">
        ...
        <div class="form-group component-style-select-div <%= this.uiconfig.isShow_style ? '' : 'hidden' %>">
            ...
            <div class="col-sm-10">
                <select class="form-control" name="componentStyle[]">
                    ...
                    <option value="<%= COMPONENT_STYLE_BOARD_SPLIT %>" <%= style == COMPONENT_STYLE_BOARD_SPLIT ? 'selected' : '' %>>分屏样式</option>
                    <option value="<%= COMPONENT_STYLE_CIRCLE %>" <%= style == COMPONENT_STYLE_CIRCLE ? 'selected' : '' %>>圈子样式</option>
                </select> 
            </div>
        </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <select class="form-control input-sm layoutStyleSelect" name="layoutStyle">
                        ...
                        <option value="<%= COMPONENT_STYLE_LAYOUT_ONE_COL_MID %>" <%= style == COMPONENT_STYLE_LAYOUT_ONE_COL_MID ? 'selected' : '' %> class="<%= this.uiconfig.isShow_layoutOneColMid ? '' : 'hidden' %>">单栏样式(中)</option>
                        ...
                        <option value="<%= COMPONENT_STYLE_LAYOUT_ONE_COL_SUPER_LOW %>" <%= style == COMPONENT_STYLE_LAYOUT_ONE_COL_SUPER_LOW ? 'selected' : '' %> class="<%= this.uiconfig.isShow_layoutOneColSuperLow ? '' : 'hidden' %>">单栏样式(超低)</option>
                        ...
                        <option value="<%= COMPONENT_STYLE_LAYOUT_TWO_COL_SUPER_LOW %>" <%= style == COMPONENT_STYLE_LAYOUT_TWO_COL_SUPER_LOW ? 'selected' : '' %> class="<%= this.uiconfig.isShow_layoutTwoColSuperLow ? '' : 'hidden' %>">双栏样式(超低)</option>
                        ...
                        <option value="<%= COMPONENT_STYLE_LAYOUT_THREE_COL_SUPER_LOW %>" <%= style == COMPONENT_STYLE_LAYOUT_THREE_COL_SUPER_LOW ? 'selected' : '' %> class="<%= this.uiconfig.isShow_layoutThreeColSuperLow ? '' : 'hidden' %>">三栏样式(超低)</option>
                        ...
                        <option value="<%= COMPONENT_STYLE_LAYOUT_FOUR_COL_SUPER_LOW %>" <%= style == COMPONENT_STYLE_LAYOUT_FOUR_COL_SUPER_LOW ? 'selected' : '' %> class="<%= this.uiconfig.isShow_layoutFourColSuperLow ? '' : 'hidden' %>">四栏样式(超低)</option>
                        ...
                        <option value="<%= COMPONENT_STYLE_LAYOUT_SLIDER_HIGH %>" <%= style == COMPONENT_STYLE_LAYOUT_SLIDER_HIGH ? 'selected' : '' %> class="<%= this.uiconfig.isShow_layoutSliderHigh ? '' : 'hidden' %>">幻灯片样式(高)</option>
                        <option value="<%= COMPONENT_STYLE_LAYOUT_SLIDER_MID %>" <%= style == COMPONENT_STYLE_LAYOUT_SLIDER_MID ? 'selected' : '' %> class="<%= this.uiconfig.isShow_layoutSliderMid ? '' : 'hidden' %>">幻灯片样式(中)</option>
                        <option value="<%= COMPONENT_STYLE_LAYOUT_SLIDER_LOW %>" <%= style == COMPONENT_STYLE_LAYOUT_SLIDER_LOW ? 'selected' : '' %> class="<%= this.uiconfig.isShow_layoutSliderLow ? '' : 'hidden' %>">幻灯片样式(低)</option>
                        ...
                        <%if(typeof NOW_VERSION != 'undefined' && NOW_VERSION >= 2){%>
                        <option value="<%= COMPONENT_TYPE_SUBNAVFLAT %>" <%= style == COMPONENT_TYPE_SUBNAVFLAT ? 'selected' : '' %> class="<%= this.uiconfig.isShow_layoutSubnavFlat ? '' : 'hidden' %>">二级导航</option>
                        <option value="<%= COMPONENT_TYPE_SEPARATOR %>" <%= style == COMPONENT_TYPE_SEPARATOR ? 'selected' : '' %> class="<%= this.uiconfig.isShow_layoutSeparator ? '' : 'hidden' %>">分割线</option>
                        <%}%>
                        <!--
    ...
    <% } else if (style == COMPONENT_STYLE_LAYOUT_ONE_COL_MID) { %>
    ...
    <% } else if (style == COMPONENT_STYLE_LAYOUT_ONE_COL_LOW) { %>
        ...
        <% var component = componentList[i].attributes; %>    
            <div class="textOverlapDown">
                <img src="<%= component['icon'] %>" style="width:320px;height:70px;" class="img-rounded">
            </div>
        <% } %>
        </div>
    <% } else if (style == COMPONENT_STYLE_LAYOUT_ONE_COL_SUPER_LOW) { %>
        <div class="custom-style-layout-one_col-low-fixed">
        <% for (var i = 0; i < componentList.length; i++) { %>
        <% var component = componentList[i].attributes; %>    
            <div class="textOverlapDown">
                <img src="<%= component['icon'] %>" style="width:320px;height:36px;" class="img-rounded">
            </div>
        <% } %>
        </div>
        ...
        <div class="layouttwocol-high">
            ...
            <% var component = componentList[i].attributes; %>
                <div class="layouttwocol-hight-img">
                    <img src="<%= component['icon'] %>" class="img-rounded" >
    ...
    <% } else if (style == COMPONENT_STYLE_LAYOUT_TWO_COL_LOW) { %>
        ...
        <% } else if (style == COMPONENT_STYLE_LAYOUT_TWO_COL_SUPER_LOW) { %>
        <div class="layout-two-col-super-low">
            <% for (var i = 0; i < componentList.length; i++) { %>
            <% var component = componentList[i].attributes; %>
                <div class="custom-style-layout-two-col-super-low">
                    <img src="<%= component['icon'] %>" class="img-rounded" >
                    <div class="custom-covering-title">
                        <div class="textoverlapdown-color"><%= component['title'] %></div>
                    </div>
        ...
        <div class="layout-three-col-high">
            ...
            <% var component = componentList[i].attributes; %>
                <div class="custom-style-layout-three-col-high">
                    <img src="<%= component['icon'] %>" class="img-rounded" style="width:95px;height:130px;">
                    <div class="custom-covering-title">
                        <div class="textoverlapdown-color"><%= component['title'] %></div>
                    </div>
                </div>
        ...
        <div class="layout-three-col-mid">
            ...
            <% var component = componentList[i].attributes; %>
                <div class="custom-style-layout-three-col-mid">
                    <img src="<%= component['icon'] %>" class="img-rounded" style="width:95px;height:100px;">
                    <div class="custom-covering-title">
                        <div class="textoverlapdown-color"><%= component['title'] %></div>
                    </div>
                </div>
        ...
        <div class="layout-three-col-low">
            ...
            <% var component = componentList[i].attributes; %>
                <div class="custom-style-layout-three-col-low">
                    <img src="<%= component['icon'] %>" class="img-rounded" style="width:95px;height:70px;">
                    <div class="custom-covering-title">
                        <div class="textoverlapdown-color"><%= component['title'] %></div>
                    </div>
                </div>
    ...
    <% } else if (style == COMPONENT_STYLE_LAYOUT_THREE_COL_SUPER_LOW) { %>
        <div class="layout-three-col-super-low">
            <% for (var i = 0; i < componentList.length; i++) { %>
            <% var component = componentList[i].attributes; %>
                <div class="custom-style-layout-three-col-super-low">
                    <img src="<%= component['icon'] %>" class="img-rounded" >
                    <div class="custom-covering-title">
                        <div class="textoverlapdown-color"><%= component['title'] %></div>
                    </div>
    ...
    <% } else if (style == COMPONENT_TYPE_SUBNAVFLAT) { %>
    <div class="layout-four-col">
        <% for (var i = 0; i < componentList.length; i++) { %>
        <% var component = componentList[i].attributes; %>
        <div class="custom-style-layout-four-col">
    ...
    <% } else if (style == COMPONENT_TYPE_SEPARATOR) { %>
    <hr/>
    <% } else if (style == COMPONENT_STYLE_LAYOUT_FOUR_COL_SUPER_LOW) { %>
        <div class="layout-four-col-super-low">
            <% for (var i = 0; i < componentList.length; i++) { %>
            <% var component = componentList[i].attributes; %>
                <div class="custom-style-layout-four-col-super-low">
                    <img src="<%= component['icon'] %>" class="img-rounded" style="width:75px;height:40px;">
                    <div class="custom-covering-title">
                        <div class="textoverlapdown-color"><%= component['title'] %></div>
                    </div>
                </div>
    ...
    <% } else if (style == COMPONENT_STYLE_LAYOUT_SLIDER_HIGH) { %>
        <div class="carousel slide carousel-example-generic_one" data-ride="carousel" data-interval="3000" style="width:337px;height:168px;">
            <!-- 圆点 -->
            <ol class="carousel-indicators">
            <% for (var i = 0; i < componentList.length; i++) { %>
                <li data-target=".carousel-example-generic_one" data-slide-to="<%= i %>" class="<%= i == 0 ? 'active' : '' %>"></li>
            ...
            <div class="carousel-inner">
                ...
                <% var component = componentList[i].attributes; %>
                <div class="item <%= i == 0 ? 'active' : '' %>">
                    <img src="<%= component.icon %>" alt="" style="width:337px;height:168px;">
                    <div class="carousel-caption">
                        <p><%= component.title %></p> 
                    </div>
                </div>
        ...
        <% } else if (style == COMPONENT_STYLE_LAYOUT_SLIDER_MID) { %>
        <div class="carousel slide carousel-example-generic_one" data-ride="carousel" data-interval="3000" style="width:337px;height:126px;">
            <!-- 圆点 -->
            <ol class="carousel-indicators">
            <% for (var i = 0; i < componentList.length; i++) { %>
                <li data-target=".carousel-example-generic_one" data-slide-to="<%= i %>" class="<%= i == 0 ? 'active' : '' %>"></li>
            ...
            <div class="carousel-inner">
                ...
                <% var component = componentList[i].attributes; %>
                <div class="item <%= i == 0 ? 'active' : '' %>">
                    <img src="<%= component.icon %>" alt="" style="width:337px;height:126px;">
                    <div class="carousel-caption">
                        <p><%= component.title %></p> 
                    </div>
                </div>
        ...
        <% } else if (style == COMPONENT_STYLE_LAYOUT_SLIDER_LOW) { %>
        <div class="carousel slide carousel-example-generic_one" data-ride="carousel" data-interval="3000" style="width:337px;height:83px;">
            <!-- 圆点 -->
            <ol class="carousel-indicators">
            <% for (var i = 0; i < componentList.length; i++) { %>
                <li data-target=".carousel-example-generic_one" data-slide-to="<%= i %>" class="<%= i == 0 ? 'active' : '' %>"></li>
            ...
            <div class="carousel-inner">
                ...
                <% var component = componentList[i].attributes; %>
                <div class="item <%= i == 0 ? 'active' : '' %>">
                    <img src="<%= component.icon %>" alt="" style="width:337px;height:83px;">
                    <div class="carousel-caption">
                        <p><%= component.title %></p> 
                    </div>
                </div>
    ...
    <div class="text-left" style="margin-bottom:5px;">
        ...
        <%if(style != COMPONENT_TYPE_SUBNAVFLAT){%>
