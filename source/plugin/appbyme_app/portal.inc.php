...
if (!submitcheck('portal_submit')) {
    ...
    switch ($anchor) {
        case 'index': PortalController::showModuleView();
            break;
        case 'source': PortalController::showSourceView();
            break;
        case 'slider': PortalController::showSliderView();
            break;
        case 'module_param': PortalController::showModuleParamView();
            break;
        default: cpmsg(Appbyme::lang('mobcent_error_portal_not_allow'), '', 'error');
            break;
    }
    ...
    switch ($anchor) {
        case 'index': PortalController::submitModule();
            break;
        case 'source': PortalController::submitSource();
            break;
        case 'slider': PortalController::submitSlider();
            break;
        case 'module_param': PortalController::submitModuleParam();
            break;
        default: cpmsg(Appbyme::lang('mobcent_error_portal_not_allow'), '', 'error');
            break;
    }
...
class PortalController {
    ...
    public static function showModuleView() {
        ...
        showsubtitle(array(
            ...
            Appbyme::lang('mobcent_portal_module_id'),
        ));
        ...
        foreach ($moduleList as $module) {
            showtablerow('', array('class="td25"', 'class="td28"'), array(
                ...
                '<p>' . $module['mid'] . '</p>',
                sprintf('<a href="%s?%s&anchor=source&moduleid=%d" target="_self" class="act">%s</a>', ADMINSCRIPT, $url, $module['mid'], Appbyme::lang('mobcent_portal_module_source_edit')
                ),
                sprintf('<a href="%s?%s&anchor=slider&moduleid=%d" target="_self" class="act">%s</a>', ADMINSCRIPT, $url, $module['mid'], Appbyme::lang('mobcent_portal_module_slider_edit')
                ),
                sprintf('<a href="%s?%s&anchor=module_param&moduleid=%d" target="_self" class="act">%s</a>', ADMINSCRIPT, $url, $module['mid'], Appbyme::lang('mobcent_portal_module_param_edit')
                ),
            ));
        }
    ...
    public static function showSliderView() {
            ...
            showtablerow('', array('class="td25"', 'class="td28"'), array(
                ...
                sprintf('
                    <select name="new_idtype">
                        ...
                        <option value="%s">%s</option>
                        <option value="%s">%s</option>
                    </select>', PortalModule::SOURCE_TYPE_AID, Appbyme::lang('mobcent_portal_module_source_type_aid'), PortalModule::SOURCE_TYPE_TID, Appbyme::lang('mobcent_portal_module_source_type_tid'), PortalModule::SOURCE_TYPE_URL, 'url', PortalModule::SOURCE_TYPE_BID, Appbyme::lang('mobcent_portal_module_source_type_bid')
                ),
        ...
        foreach ($sliderList as $slider) {
            if ($slider['idtype'] == PortalModule::SOURCE_TYPE_BID) {
                ...
                echo sprintf(
                        '<div class="slider_image"><a href="%s" target="_blank"><img src="%s" alt="%s"><span>%s</span><em></em></a></div>', $slider['url'], $slider['imgurl'], $slider['title'], $slider['title']);
            }
        }
        showtagfooter('div');
    ...
    public static function submitSlider() {
        ...
        if (!empty($_POST['displayorder_new'])) {
            foreach ($_POST['displayorder_new'] as $id => $displayorder) {
                $source = PortalModule::getSourceInfo(
                                $_POST['id_new'][$id], $_POST['idtype_new'][$id], $_POST['imgid_new'][$id], $_POST['imgtype_new'][$id], $_POST['title_new'][$id]
                );
                $source = array_merge($source, array('displayorder' => $displayorder,));
                PortalModule::updateSource($id, $source);
            }
        ...
        if (!empty($_POST['new_id']) && !empty($_POST['new_imgid'])) {
            $source = PortalModule::getSourceInfo(
                            $_POST['new_id'], $_POST['new_idtype'], $_POST['new_imgid'], $_POST['new_imgtype'], $_POST['new_title']
            );
            $source = array_merge($source, array(
                'mid' => $mid,
                'displayorder' => (int) $_POST['new_displayorder'],
...
class PortalModule {
    ...
    public static function getSourceCount($mid, $type = self::SOURCE_TYPE_NORMAL, $params = array('idtype' => array(
